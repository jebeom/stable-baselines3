...
SelfVGD = TypeVar("SelfVGD", bound="VGD")


class VGD(OffPolicyAlgorithm):
    """
    ...
    def _guided_decode(self, obs: th.Tensor, initial_noise: th.Tensor, deterministic: bool = True) -> th.Tensor:
        ...
        for i, t in enumerate(t_all):
            ...
            if apply_guidance:
                ...
                grad_norm = th.linalg.vector_norm(grad_flat, dim=1).mean().item()
                delta_norm = th.linalg.vector_norm(delta, dim=1).mean().item()
                weighted_grad_norm = lam_t * grad_norm
                cos = F.cosine_similarity(grad_flat, delta, dim=1, eps=1e-8)
                cos_mean = cos.mean().item()
                cos_pos = (cos > 0).float().mean().item()
                with th.no_grad():
                    a_post = x.reshape(B, -1)
                ...
                sum_ratio += (weighted_grad_norm / (delta_norm + 1e-8))
                sum_cos += cos_mean
                count_cos_pos += cos_pos
                # To report delta_q, compute q_pre using z_det
                with th.no_grad():
